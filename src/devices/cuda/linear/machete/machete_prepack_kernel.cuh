...
static void prepack_B_template(
  ...
  if (!(size<0>(B_layout) % size<0>(TileShapeNKL{}) == 0)) throw std::runtime_error("Machete prepack alignment mismatch");
  if (!(size<1>(B_layout) % size<1>(TileShapeNKL{}) == 0)) throw std::runtime_error("Machete prepack alignment mismatch");
