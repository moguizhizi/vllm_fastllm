/*
 * FastLLM raw Marlin integration.
 *
 * The Marlin kernel body and GPTQ repack kernel are adapted from vLLM's
...
#define TORCH_CHECK(cond, ...)                                                   \
    do {                                                                         \
        if (!(cond)) {                                                           \
            std::printf("FastLLM Marlin check failed: %s (%s:%d)\n", #cond,      \
                        __FILE__, __LINE__);                                     \
            std::abort();                                                        \
        }                                                                        \
    } while (0)
...
using ScalarTypeId = int;

struct ScalarType {
  int bits;
  int bias;

  __host__ __device__ constexpr ScalarTypeId id() const { return bits * 256 + bias; }
  __host__ __device__ static constexpr ScalarType from_id(ScalarTypeId id) {
    return ScalarType{id / 256, id % 256};
  }
  __host__ __device__ constexpr int size_bits() const { return bits; }
  __host__ __device__ constexpr bool operator==(ScalarType const& other) const {
    return bits == other.bits && bias == other.bias;
  }
  __host__ __device__ constexpr bool operator!=(ScalarType const& other) const {
    return !(*this == other);
  }
  std::string str() const {
    std::string ret = "uint" + std::to_string(bits);
    if (bias != 0) {
...
static inline constexpr ScalarType kU4{4, 0};
static inline constexpr ScalarType kU4B8{4, 8};
static inline constexpr ScalarType kU8{8, 0};
static inline constexpr ScalarType kU8B128{8, 128};
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* s,
  ...
  for (int i = 0; i < tot_m_blocks; i += exec_cfg.max_m_blocks) {
    ...
    FASTLLM_INT4GROUP_CALL_IF(16, 4, 256)
    FASTLLM_INT4GROUP_CALL_IF(8, 8, 256)
    FASTLLM_INT4GROUP_CALL_IF(8, 4, 128)
    FASTLLM_INT4GROUP_CALL_IF(4, 8, 128)
    else {
...
#define CALL_IF(NUM_BITS, HAS_PERM)                                           \
    ...
    marlin::gptq_marlin_repack_kernel<marlin::repack_threads, NUM_BITS,       \
        ...
        <<<blocks, marlin::repack_threads, max_shared_mem, stream>>>(         \
            b_q_weight_ptr, nullptr, out_ptr, size_k, size_n);                \
  }

static bool FastllmCudaMarlinCurrentDeviceSupported() {
    int dev = 0;
    int major = 0, minor = 0;
...
extern "C" bool FastllmCudaMarlinHalfInt4Gemm(const void *a, const uint32_t *b_q_weight,
    ...
    marlin::marlin_mm<half>(
        ...
        vllm::kU4, false, false, true, num_groups, group_size, dev, 0,
