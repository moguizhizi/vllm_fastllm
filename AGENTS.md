...
- 测试用例应尽可能与正式推理主流程保持一致，优先调用相同的公开入口、后端选择、内存布局和同步边界。尤其是性能测试，禁止为了提高测试成绩而仅在测试代码中绕过检查、缓存、调度、同步或其他主流程开销；性能优化应先落实到正式主流程，再由测试用例验证。确需比较实验版本时，应使用独立且明确标识的微基准，不得用其实验结果替代正式主流程的性能结果；与其他框架比较时还必须统一计时范围、warmup、迭代次数和同步方式。
