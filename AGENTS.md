...
- 默认禁止在当前本地工作区执行编译，包括但不限于 CMake、Make、Ninja、
  NVCC、C/C++ 编译器、项目构建脚本，以及任何会间接触发 C/C++/CUDA
  编译或链接的测试、安装和验证命令。
- 只有用户在当前请求中明确要求或明确授权本地编译时，才可以执行上述命令；
  过去请求中的一次性授权不得自动沿用到后续请求。
- 未获得本地编译授权时，只能进行不触发编译的检查，例如 Python 语法检查、
  文本搜索、差异检查、格式检查和已有产物检查，并在结果中明确说明未编译。
- 可以为用户提供在其服务器上执行的编译命令，但不得因此在本地代为执行。
...
- 回答中只要引用代码文件、函数、类型、变量或具体实现依据，就必须同时
  给出对应的具体行号，不能只提供文件名或模糊目录。
- 行号必须直接出现在用户可见的文本中，不得只隐藏在Markdown链接
  目标里。链接的可见标签至少应显示`文件名:行号`；也可在正文中
  明确写出“第N行”。
- 行号必须以回答时当前工作区的实际文件内容为准；引用前应重新查询，不能
  沿用可能因后续修改而失效的历史行号。
- 同一文件涉及多个不同实现位置时，应分别标明各自的起始行号，使每项结论
  都能直接定位到对应代码。
- 如果文件尚未生成、来自服务器外部环境或当前无法取得可靠行号，应明确
  说明原因，不得编造行号。
- 正确示例：`Prefix Cache在 [basellm.cpp:849](/abs/path/src/models/basellm.cpp:849) 限制恢复长度。`
- 错误示例：`Prefix Cache在 [basellm.cpp](/abs/path/src/models/basellm.cpp:849) 限制恢复长度。`
  错误示例虽然链接目标包含行号，但渲染后用户只能看到文件名。
