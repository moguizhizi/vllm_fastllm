...
- 函数体应按职责和执行阶段组织代码，例如类型定义、参数检查、输入与布局
  准备、主流程参数构造、资源准备、执行或提交、错误处理及结果返回。
- 不同逻辑阶段之间使用一个空行分隔，使代码层次和执行流程清晰；同一阶段
  内紧密关联的语句应连续书写，不在每条语句之间机械插入空行。
- 较长函数应优先通过逻辑分段提升可读性；仅调整格式时不得改变执行顺序、
  生命周期、同步边界、错误处理或回退语义。
- 空行用于表达逻辑边界，不得堆叠多个连续空行，也不得用空行掩盖本应拆分
  为独立函数的复杂职责。
